...
class DetachedHelper
{
    ...
    const CORE_MODULES = [
        'Activities', 'Auth', 'Billable', 'Brands', 'Calls', 'Comments', 'Contacts', 'Core', 'Deals',
        'Documents', 'Installer', 'MailClient', 'Notes', 'ThemeStyle', 'Translator', 'Updater', 'Users', 'WebForms',
    ];
    ...
    public static function hasCoreModulesDisabled()
    {
        ...
        foreach (static::CORE_MODULES as $moduleName) {
            if (! isset($modules[$moduleName]) ||
                 $modules[$moduleName] == false ||
                ! is_dir(__DIR__.'/modules/'.$moduleName)) {
                return true;
            }
        }
